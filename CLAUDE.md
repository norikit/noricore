# CLAUDE.md — entry point for AI agents working in noricore

**You are working on `noricore` (a project of the `norikit` org): the event broker and data backbone of the norikit ecosystem.**
...
- **D1 — macOS-first, Swift daemon.** noricore is a native Swift process; no cross-platform
  abstraction layers. See [ai-docs/decisions.md](ai-docs/decisions.md).
- **D2 — Standalone daemon, not embedded.** noricore runs as an independent process
  (LaunchAgent); it is not bundled inside any tool.
- **D3 — macOS 13 floor.** Consistent with noribar; availability-gate newer APIs.
...
- Match surrounding code style; favor clarity and low-latency code.
